...
async def publish_distance(channel):
    arduino = serial.Serial('COM5', 9600)  # Replace 'COM5' with the appropriate port for your Arduino
    while True:
        distance = arduino.readline().decode().strip()  # Read the distance value from the Arduino
        await channel.publish(name='distance', data={'value': distance})
        print('Published distance:', {'value': distance})
...
async def main():
    ...
    await publish_distance(channel)
